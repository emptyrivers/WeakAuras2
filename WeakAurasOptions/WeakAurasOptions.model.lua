--code model of WeakAuras

-- retained for luacheck
local L = WeakAuras.L
...
-- cache of display tab options for each aura
...
-- transient cache similar to Custom Option Array pages
local optionTriggerChoices = {};
WeakAuras.optionTriggerChoices = optionTriggerChoices;

-- cache of thumbnail objects each one contains an object created by regionOptions.createThumbnail
WeakAuras.thumbnails = {};
local thumbnails = WeakAuras.thumbnails;

-- cache of option tables fed to AceConfig for each aura
local displayOptions = {};
WeakAuras.displayOptions = displayOptions;

-- cache of auras to be displayed in the "loaded" section on LHS
local loaded = WeakAuras.loaded;

-- persistent cache of spellID&spellIcon, created in Cache.lua
local spellCache = WeakAuras.spellCache;

-- alternate way of locating WeakAurasSaved/WeakAurasOptionsSaved
local savedVars = {};
WeakAuras.savedVars = savedVars;

-- HACK used for multi-selecting
-- Contents: bogus data to fool AddOption into building a merged list of whatever we want
local tempGroup = {}
WeakAuras.tempGroup = tempGroup

-- Callback used in Context menu of AceGUIWidget-WeakAurasDisplayButton.lua OnDuplicateClick
-- Duplicates selection and creates new displaybuttons as appropriate.
-- Calls: WeakAuras.Add(newData)
--        WeakAuras.NewDisplayButton(newData)
--        WeakAuras.Add(parentData)
--        WeakAuras.UpdateDisplayButton(parentData)
--        WeakAuras.ReloadGroupRegionOptions(parentData)
-- Returns: new id of duplicated data
function WeakAuras.DuplicateAura(data, newParent)
end

-- helper function that creates a table with all keys of table1, table2
-- if both table1 and table2 have a key, then the value is that of table2
-- otherwise, the value of the respective table is used
local function union(table1, table2)
end

-- LHS GUI layout
-- everything is directly under the previous item in the list
AceGUI:RegisterLayout("AbsoluteList", function(content, children) end)
